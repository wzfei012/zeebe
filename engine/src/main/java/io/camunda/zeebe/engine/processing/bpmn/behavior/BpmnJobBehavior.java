...
public final class BpmnJobBehavior {
  ...
  private Either<Failure, JobProperties> evaluateJobExpressions(
    ...
    return Either.<Failure, JobProperties>right(new JobProperties())
        ...
        .flatMap(p -> evaluateAssigneeExpression(p, assignee, scopeKey));
  }

  private Either<Failure, JobProperties> evaluateAssigneeExpression(
      final JobProperties jobProperties, final Expression assignee, final long scopeKey) {
    if (assignee == null) {
      return Either.right(jobProperties);
    }
    return expressionBehavior
        .evaluateStringExpression(assignee, scopeKey)
        .map(jobProperties::assignee);
  }
  ...
  private DirectBuffer encodeHeaders(final Map<String, String> taskHeaders, final String assignee) {
    ...
    if (assignee != null) {
      headers.put(Protocol.TASK_ASSIGNEE_HEADER_NAME, assignee);
    }
